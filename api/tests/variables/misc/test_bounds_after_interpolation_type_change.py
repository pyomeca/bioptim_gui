...
from bioptim_gui_api.variables.misc.variables_utils import bounds_after_interpolation_type_change
...
def test_constant_to_constant(default_const_variable):
    bounds_after_interpolation_type_change(default_const_variable, "CONSTANT")
    assert np.allclose(
    ...
    assert default_const_variable["initial_guess"] == [
        [-1.0],
        [-2.0],
        [-3.0],
        [-4.0],
...
def test_constant_to_linear(default_const_variable):
    bounds_after_interpolation_type_change(default_const_variable, "LINEAR")
    assert np.allclose(
        default_const_variable["bounds"]["min_bounds"],
        [
            [-1.0, -1.0],
...
def test_constant_to_constant_with_first_and_last_different(default_const_variable):
    bounds_after_interpolation_type_change(default_const_variable, "CONSTANT_WITH_FIRST_AND_LAST_DIFFERENT")
    assert np.allclose(
        default_const_variable["bounds"]["min_bounds"],
        [
            [-1.0, -1.0, -1.0],
...
def test_linear_to_constant(default_linear_variable):
    bounds_after_interpolation_type_change(default_linear_variable, "CONSTANT")
    assert np.allclose(
    ...
    assert default_linear_variable["initial_guess"] == [
        [-1.0, 1.0],
        [-2.0, 2.0],
        [-3.0, 3.0],
        [-4.0, 4.0],
...
def test_linear_to_linear(default_linear_variable):
    bounds_after_interpolation_type_change(default_linear_variable, "LINEAR")
    assert np.allclose(
        default_linear_variable["bounds"]["min_bounds"],
        [
            [-1.0, 1.0],
...
def test_linear_to_constant_with_first_and_last_different(default_linear_variable):
    bounds_after_interpolation_type_change(default_linear_variable, "CONSTANT_WITH_FIRST_AND_LAST_DIFFERENT")
    assert np.allclose(
        default_linear_variable["bounds"]["min_bounds"],
        [
            [-1.0, -1.0, 1.0],
...
def test_constant_with_first_and_last_different_to_constant(default_cwfld_variable):
    bounds_after_interpolation_type_change(default_cwfld_variable, "CONSTANT")
    assert np.allclose(
    ...
    assert default_cwfld_variable["initial_guess"] == [
        [-1.0, 1.0, 1.0],
        [-2.0, 2.0, 2.0],
        [-3.0, 3.0, 3.0],
        [-4.0, 4.0, 4.0],
...
def test_constant_with_first_and_last_different_to_linear(default_cwfld_variable):
    bounds_after_interpolation_type_change(default_cwfld_variable, "LINEAR")
    assert np.allclose(
        default_cwfld_variable["bounds"]["min_bounds"],
        [
            [-1.0, 1.0],
    ...
    assert default_cwfld_variable["initial_guess_interpolation_type"] == "CONSTANT_WITH_FIRST_AND_LAST_DIFFERENT"
    assert default_cwfld_variable["initial_guess"] == [
        [-1.0, 1.0, 1.0],
        [-2.0, 2.0, 2.0],
        [-3.0, 3.0, 3.0],
...
def test_constant_with_first_and_last_different_to_constant_with_first_and_last_different(default_cwfld_variable):
    bounds_after_interpolation_type_change(default_cwfld_variable, "CONSTANT_WITH_FIRST_AND_LAST_DIFFERENT")
    assert np.allclose(
        default_cwfld_variable["bounds"]["min_bounds"],
        [
            [-1.0, -1.5, 1.0],
