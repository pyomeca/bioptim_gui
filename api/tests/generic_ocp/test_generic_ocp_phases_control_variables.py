...
def test_put_control_variable_dimension():
    response = client.put(
        "/generic_ocp/phases_info/0/control_variables/0/dimension",
        json={"dimension": 2},
    )
...
def test_put_control_variable_bounds(bounds):
    response = client.put(
        f"/generic_ocp/phases_info/0/control_variables/0/{bounds}",
        json={
            "x": 0,
            "y": 0,
            "value": 42,
    ...
    response = client.get("/generic_ocp/phases_info/")
    phase_info = response.json()[0]
    assert phase_info["control_variables"][0]["bounds"][f"{bounds}"] == [
        [42.0, 0.0, 0.0]
    ]
