...
def plot_solutions(pkl_paths: list[str]) -> None:
    assert len(pkl_paths) != 0
    
    with open(pkl_paths[0], 'rb') as f:
        o = pkl.load(f)

    nb_q = len(o["integrated_states"][0]['q'])
    name_dof = get_dofs_name(nb_q)

    final_time = round(o["time_vector"][0][-1], 2)
    nb_phases = len(o["solution"].states)
    n_shooting = [len(o["solution"].states[phase]['q'][0]) - 1 for phase in range(nb_phases)]
    ...
    for i, pkl_path in enumerate(pkl_paths):
        ...
        for dof in range(nb_q):
            solution_q = o["integrated_states"][0]['q'][dof]

            # plot solution
            color = cmap(i / len(pkl_paths))
            axs[dof].plot(time, solution_q, color=color)
            ...
            min_bounds_per_phase = [o["x_bounds"][phase]['q'].min[dof].tolist() for phase in range(nb_phases)]
            max_bounds_per_phase = [o["x_bounds"][phase]['q'].max[dof].tolist() for phase in range(nb_phases)]
            ...
            for phase in range(nb_phases):
                ...
                min_bounds = min_bounds_per_phase[phase]
                max_bounds = max_bounds_per_phase[phase]
                ...
                for bound in [min_bounds, max_bounds]:
                    axs[dof].hlines(bound[0], t_start, t_start + bound_width, linewidth=0.5, color='black')
                    axs[dof].vlines(t_start + bound_width, bound[0], bound[1], linewidth=0.5, color='black')
                    
                    axs[dof].hlines(bound[1], t_start + bound_width, t_end - bound_width, linewidth=0.5, color='black')
                    axs[dof].vlines(t_end - bound_width, bound[1], bound[2], linewidth=0.5, color='black')
                    
                    axs[dof].hlines(bound[2], t_end - bound_width, t_end, linewidth=0.5, color='black')
    ...
    outpath = Path(pkl_path).parent / 'q_graphs.png'
